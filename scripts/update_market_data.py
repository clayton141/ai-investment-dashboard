...
def fundamentals(symbol):
    t = yf.Ticker(symbol)
    inc, cf = t.quarterly_financials, t.quarterly_cashflow
    rev = row(inc, ("Total Revenue","Operating Revenue"))
    rev_ttm = ttm(rev)
    growth = yoy(rev)
    fcf = ttm(row(cf, ("Free Cash Flow",)))
    if fcf is None:
        ocf = ttm(row(cf, ("Operating Cash Flow","Total Cash From Operating Activities")))
        capex = ttm(row(cf, ("Capital Expenditure","Capital Expenditures")))
    ...
    try:
        ...
        info = {}
    try:
        mcap = num(t.fast_info.get("market_cap"))
    except Exception:
        mcap = None
    mcap = mcap or num(info.get("marketCap"))
    ev, fpe = num(info.get("enterpriseValue")), num(info.get("forwardPE"))
    out = {}
...
def main():
    ...
    for s in new.get("watchlist", []):
        try:
            histories[s["ticker"]] = history(s["ticker"], expected)
    ...
    for s in new.get("watchlist", []):
        symbol = s["ticker"]
        s.update(technicals(histories[symbol]))
        try:
            s.update(fundamentals(symbol))
        except Exception as e:
            warnings[symbol] = str(e)
    ...
    new["automation"] = {
        ...
        "preservedFields": [
            ...
            "thesis","risk","bearPct","basePct","bullPct","ARR/cRPO/billings"
        ],
    }
    if warnings:
        new["automation"]["warnings"] = warnings
    ...
    if a == b:
        ...
        return
    new["updatedAt"] = datetime.now(TAIPEI).strftime("%Y-%m-%d %H:%M Asia/Taipei")
    DATA.write_text(json.dumps(new, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
